...
```bash
# Build image
docker build -t jobscamai:latest .

# Run container
docker run -p 5000:5000 jobscamai:latest
...
4. Configure:
   ...
   - **Start Command**: `gunicorn -w 4 -b 0.0.0.0:5000 -t 600 app:app`
...
2. **Clone your fork**
   ```bash
   git clone https://github.com/YOUR_USERNAME/JobScamAI.git
   cd JobScamAI
   ```
...
- ✨ **Features**: Add new detection signals
...
### Feature Request?
Create an issue with the tag `enhancement` and describe:
